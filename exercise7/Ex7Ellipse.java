...
/**
 * The class to set up and draw the ellipse.
 * 
 * @author Chau
 * 
 */
public class Ex7Ellipse extends Ex7DrawingElement {
	 ...
	 *            height of the shape.
	 */
	protected Ex7Ellipse(int left, int top, int width, int height) {
	...
	public void Draw(Graphics drawingElement) {
		...
		if (x1 < x2 && y1 < y2) {
			...
			drawingElement.drawOval(x1, y1, x2, y2);
		} else if (x1 > x2 && y1 > y2) {
			// reverse the coordinates.
			x1 = x1 - x2;
			y1 = y1 - y2;

			drawingElement.drawOval(x2, y2, x1, y1);
		} else if (x1 > x2 && y1 < y2) {
			// other way round.
			x1 = x1 - x2;
			y2 = y2 - y1;

			drawingElement.drawOval(x2, y1, x1, y2);
		} else if (x1 < x2 && y1 > y2) {
			// (alternate) other way round.
			x2 = x2 - x1;
			y1 = y1 - y2;

			drawingElement.drawOval(x1, y2, x2, y1);
		} else if (x1 == x2 && y1 == y2) {
			// draw no shape.
			x2 = x1 - x2;
			y2 = y1 - y2;
